...
/// `ludwig-spec: <id>@<version> hash=<sha>` — captures id, version, hash.
...
pub fn strip_trailing_comment(content: &str) -> String {
    // Remove the line containing `ludwig-spec:` if it is among the last few lines.
    ...
    while let Some(last) = lines.last() {
        ...
        if TRAILING_COMMENT_RE.is_match(last) {
            lines.pop();
        }
        break;
    }
...
fn matched_files(project: &Project, pattern: &str) -> Vec<PathBuf> {
    if plan::contains_glob(pattern) {
        ...
        if p.is_file() { vec![p] } else { vec![p] /* still report as missing */ }
    }
}
